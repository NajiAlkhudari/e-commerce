...
//     // حذف جميع عناصر العربة من قاعدة البيانات
//     await CartItem.deleteMany({ _id: { $in: cart.cart_items.map(item => item._id) } });
...
export async function POST(req, { params }) {
  ...
  try {
    ...
    // التحقق من أن العربة تحتوي على عناصر
    if (!cart || !cart.cart_items || cart.cart_items.length === 0) {
      return new Response(JSON.stringify({ message: "Cart is empty or invalid" }), { status: 400 });
    }
    ...
    // إضافة تفاصيل الطلب إلى الطلب الجديد
    newOrder.order_items = orderItems.map(item => item._id);
    await newOrder.save();
